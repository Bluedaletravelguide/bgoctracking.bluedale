...
<body>
    ...
    <div class="bar">
        <div class="company">
            ...
            <div class="small">No. 312, Blok F2, Level 2, Jalan PJU 1/42A, Dataran Prima, 47301 Petaling Jaya, Selangor
                Darul Ehsan, Malaysia.</div>
            <div class="small">Tel: +603 - 7886 9219 | Fax: +603 - 7887 8212 | Website: www.bluedale.com.my | Email:
                enquiry@bluedale.com.my</div>
        </div>
    ...
    @php
        ...
        $product = strtoupper($file->product ?? '');

        $fullName = $map[$product] ?? $product; // fallback to original if not in map
    @endphp

    <div class="title">{{ $fullName }} JOB ORDER</div>
    <table class="meta">
        <tr>
            <td class="left">&nbsp;</td>
    ...
    <table style="margin-top:6px;" class="job-rows">
        ...
        @for ($i = 0; $i < $total; $i++)
            ...
            <tr>
                <td>{{ $it->site ?? '' }} - {{ $it->billboard?->location?->name ?? '' }}</td>
                {{-- <td>{{ $it->size ?? '' }}</td> --}}
                <td>
                    {{ $it?->size? collect(explode('x', $it->size))->map(fn($v) => $v . 'ft')->implode(' x '): '' }}
                </td>
